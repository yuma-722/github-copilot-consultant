...
| Claude 形式のファイル別ルール | `.claude/rules/*.md` | `paths` に一致するファイル作業時 |
...
- Claude 形式の `.claude/rules/*.md` では `applyTo` ではなく `paths` を使う。
- `paths` は glob 配列で、未指定時は `**`（全ファイル）として扱われる。
- instructions の探索場所は `chat.instructionsFilesLocations` で追加できる。
- `.github/instructions/` と `~/.copilot/instructions/` は再帰的に探索される。
...
モノレポでサブフォルダを開く運用なら、`chat.useCustomizationsInParentRepositories` により親リポジトリの instructions を拾える。

### 7) 確認とデバッグ

- Chat Customizations editor（Preview）を開くと、Instructions タブで作成と編集をまとめて行える。
...
- `/create-instruction` は targeted な `.instructions.md` を作る用途、`/create-instructions` と `/init` は workspace 全体の常時指示の生成用途として使い分ける
...
- settings ベースの code generation / test generation instructions は deprecated。新規作成は file-based instructions を優先する
