...
追加の探索場所は `chat.skillsLocations` で設定できる。モノレポで親リポジトリの Skill も拾いたい場合は `chat.useCustomizationsInParentRepositories` を確認する。
...
- `/skills` で Configure Skills メニューを開ける
- `/create-skill` や Chat Customizations editor（Preview）の Generate Skill で初期ドラフトを生成できる
