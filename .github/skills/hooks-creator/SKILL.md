...
| ワークスペース（Claude 互換） | `.claude/settings.local.json`, `.claude/settings.json` |
| ユーザー全体 | `~/.copilot/hooks`, `~/.claude/settings.json` |
| Custom Agent 単位 | `.agent.md` frontmatter の `hooks` |

ワークスペース hooks がユーザー hooks より優先される。通常は `.github/hooks/` に JSON ファイルを作成する。追加の探索場所は `chat.hookFilesLocations` で制御できる。

### 2) フック設定ファイルを書く

JSON 形式で `hooks` オブジェクトを記述する。VS Code は Claude Code / Copilot CLI の hook format と互換のある JSON を読み込める:
...
- `/create-hook` を使うとイベント種別と雛形を含む初期設定を生成できる
- Chat Customizations editor（Preview）から hooks の一覧と編集を確認できる
- 出力パネルで **GitHub Copilot Chat Hooks** チャンネルを確認する
- Chat ビューを右クリック → **Diagnostics** でフックの読み込み状態を確認する
- スクリプトに実行権限があるか確認する（`chmod +x`）
- agent-scoped hooks を使う場合は `chat.useCustomAgentHooks` が有効か確認する
...
- Claude 形式の matcher は VS Code では現在無視されるため、対象ツールの絞り込みはスクリプト内で判断する
