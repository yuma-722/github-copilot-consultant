...
## 3.7) Agent-scoped hooks（Preview）
...
description: 変更後にフォーマットを補助する agent-scoped hooks 付き agent
...
- この agent が有効なときだけ hook を実行する
- `chat.useCustomAgentHooks` が無効なら、workspace hooks に寄せる
```
