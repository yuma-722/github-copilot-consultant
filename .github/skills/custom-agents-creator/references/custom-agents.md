...
- Claude 互換形式: `.claude/agents/*.md`
- ユーザープロファイル: `~/.copilot/agents/` または VS Code のユーザーデータ配下（複数ワークスペースで再利用）
- 追加ロケーション: `chat.agentFilesLocations` で検索場所を増やせる

補足:

- VS Code は `.github/agents/` 配下の `.md` も検出するが、明示性のため `.agent.md` を推奨する。
- モノレポでは `chat.useCustomizationsInParentRepositories` を有効にすると親リポジトリの agent も探索できる。
...
- `name`: 表示名（省略時はファイル名）
- `description`: 目的/役割。チャット入力欄のプレースホルダーにも使われる
- `argument-hint`: 入力のヒント（VS Code向け）
- `tools`: 利用可能ツールの絞り込み（存在しないツール名は無視される）
- `agents`: subagentとして使ってよいエージェントの許可リスト（`*` / `[]` / 名前配列、既定は `*`）
...
- `hooks`: この agent にだけ適用する hooks（Preview。`chat.useCustomAgentHooks` が必要）
- `target`: 対象環境（`vscode` または `github-copilot`）
- `mcp-servers`: GitHub Copilot 向け custom agent で使う MCP server 設定
- `infer`: deprecated。互換用途のみ

`handoffs` の子フィールド:

- `handoffs.label`: ボタン表示名
- `handoffs.agent`: 移動先 agent identifier
- `handoffs.prompt`: handoff 時に渡すプロンプト
- `handoffs.send`: その場で自動送信するか（既定 `false`）
- `handoffs.model`: handoff 実行時に使うモデル
...
- subagent として使う custom agent は、`user-invocable: false` にして picker から隠し、Coordinator からだけ使えるようにする構成が扱いやすい
...
- 旧 `.chatmode.md` は custom agents へ移行済み。更新時は `.agent.md` に寄せる。
- そのため、まずは `description` / `name` / `tools` と本文指示で成立するエージェントを作り、必要に応じて追加項目を使う。
