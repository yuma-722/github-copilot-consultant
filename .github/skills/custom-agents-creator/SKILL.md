...
description: VS Code と GitHub Copilot の Custom Agents（カスタムエージェント）を設計・作成・レビューする。`.agent.md`、`.github/agents`、YAML frontmatter（name/description/argument-hint/tools/agents/model/user-invocable/disable-model-invocation/handoffs/hooks/target/mcp-servers）、subagents（サブエージェント）や handoffs を扱うタスク、または Copilot coding agent の custom agents configuration に言及したときに使用する。
---
...
- **個人用に再利用する**: `~/.copilot/agents/` または VS Code のユーザーデータ配下
...
必要なら `chat.agentFilesLocations` で探索場所を増やす。モノレポでサブフォルダだけ開く運用なら、`chat.useCustomizationsInParentRepositories` も確認する。
...
- subagents を使う場合は、Coordinator（調整役）と Worker（専門役）に分け、Coordinator側で `agents` を絞る。
  ...
  - `agents` の既定挙動は `*` で、`[]` を指定すると subagent 利用を禁止できる。
- preview の `hooks` を frontmatter に持たせる場合は、`chat.useCustomAgentHooks` が有効か確認する。

### 4) `.agent.md` を生成する

- ファイル名（例）: `.github/agents/<agent-id>.agent.md`
- YAML frontmatter は **最低限 `description` を入れ、必要なら `name` で表示名を固定する**。
- 可能なら `argument-hint` を入れて入力の期待値を明確にする（VS Code向け）。
- handoffs を使う場合は `label` / `agent` / `prompt` / `send` / `model` を必要最小限だけ書く。
...
- Chat Customizations editor（Preview）または `/agents` で Custom Agents の一覧を確認する。
- `/create-agent` を使うと初期ドラフトを生成できるが、そのまま採用せず `tools` と `agents` を絞る。
...
- 未対応の frontmatter は実行環境によって無視されることがある（困ったら `description` / `name` / `tools` を核にして組む）。
- `infer` は deprecated。VS Code では `user-invocable` / `disable-model-invocation` を優先する。
- agent-scoped hooks は Preview。安定前提で必須機能のように設計しない。
- 旧 `.chatmode.md` は custom agents にリネームされた。新規作成や説明は `.agent.md` 基準にそろえる。
